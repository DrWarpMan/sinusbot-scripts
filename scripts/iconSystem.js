registerPlugin({
    ...
    const { commandIcon, limitDefault, vipGroupID, limitVIP, extraVipGroupID, limitExtraVIP, blacklistGroups, icons, logEnabled } = config;
    ...
    event.on("chat", ({ client, text, mode }) => {
        ...
        if (command === commandIcon) {
            ...
            if (icons.includes(iconName)) {
                const group = backend.getServerGroups().find(g => g.name() === iconName);
                if (!group) return client.chat("Error, contact admin!");

                const groupHas = client.getServerGroups().map(g => g.id()).includes(group.id());
    ...
    function getIconLimit(client) {
        ...
        if (groups.some(gID => extraVipGroupID == gID))
            return limitExtraVIP;
        else if (groups.some(gID => vipGroupID == gID))
            return limitVIP;
