registerPlugin(
	{
		...
		vars: [
			...
			},
			{
				...
				type: "password",
				title:
					'Password [Command: "!ocr <password> <task>"] (possible tasks are: reset_global, reset_daily, reset_today, reset_visits, reset_all):',
				default: "",
			},
			{
				name: "channels",
				...
				vars: [
					...
					},
					{
						...
						type: "number",
						title:
							"How many days back, should be shown in the channel description? [EXCLUDING TODAY] (Max. 59, Min. 0):",
					},
					{
			...
			{
				name: "allowDuplicateIPs",
				type: "checkbox",
				title: "Allow duplicate IPs?",
				default: false,
			},
		],
	},
	(_, config, { name, version, author }) => {
		...
		function updateChannels() {
			...
			channels.forEach(
				({
					...
					topic,
					description,
					descriptionDays: days,
					descriptionDate: dateFormat,
					headerUniqueVisits: headerUV,
					headerDate: headerD,
					headerRecordOnline: headerRO,
				}) => {
		...
		function visited(visitsToday, uid, ip) {
			return visitsToday.some(
				clientData =>
					Object.keys(clientData).includes(uid) ||
					(Object.values(clientData).includes(ip) && !allowDuplicateIPs)
			);
		}
		...
		function checkGlobalRecord() {
			...
			if (record >= online)
				logMsg(
					`[GLOBAL RECORD] Checking - (OLD RECORD) ${record} > ${online} (ONLINE), record not beaten.. continuing!`
				);
			else {
				logMsg(
					`[GLOBAL RECORD] Checking - (OLD RECORD) ${record} < ${online} (ONLINE), record beaten.. saving!`
				);
				record = online;
		...
		function checkDailyRecords() {
			...
			if (todaysRecord >= online)
				logMsg(
					`[DAILY RECORDS] Checking - (OLD RECORD) ${todaysRecord} > ${online} (ONLINE), record not beaten.. continuing!`
				);
			else {
				logMsg(
					`[DAILY RECORDS] Checking - (OLD RECORD) ${todaysRecord} < ${online} (ONLINE), record beaten.. saving!`
				);
				todaysRecord = online;
		...
		function removeDuplicits(clients) {
			...
			clients.forEach(client => {
				...
				if (cIp) {
					if (
						!uniqueClients.some(uniqueClient => {
		...
		function isIgnored(client) {
			return (
				(ignoredUIDs || []).includes(client.uid()) ||
				client
					.getServerGroups()
					.map(g => g.id())
					.some(gID => (ignoredGroupIDs || []).includes(gID))
			);
		}
		...
		function checkConnection() {
			setTimeout(() => {
				if (backend.isConnected()) isConnected = true;
			}, 10 * 1000);
		}
